...
@class AMWebBrowserController;

@protocol AMWebBrowserControllerDelegate<NSObject>
...
@interface AMWebBrowserController : UIViewController <UIWebViewDelegate> {
  id<AMWebBrowserControllerDelegate> delegate;
  UIViewController *viewControllerForPresenting;
  NSArray *loadingButtons;
  NSArray *loadedButtons;
  AMCustomAdWebViewAnimType transitionType;
...
@property (nonatomic,assign) id<AMWebBrowserControllerDelegate> delegate;
...
- (void)presentWithController:(UIViewController *)viewController transition:(AMCustomAdWebViewAnimType)animType;
...
@interface AMBackButton : UIBarButtonItem
@end
