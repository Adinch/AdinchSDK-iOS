...
/**
 * If you are running your own AM server instance, make sure you
...
/**
 * You can listen to callbacks from AM via these methods.  When AM is
 ...
 * AMView object as a subview to your view.  This view contains the ad.
 * When you are notified that an ad request failed, you are also informed if the
 * AMView is fetching a backup ad.  The backup fetching order is specified
 * by you in AM.com or your own server instance.  When all backup sources
 ...
 * request another AMView via [AMView requestAMViewWithDelegate:]
 */
- (void)AMDidReceiveAd:(AMView *)amView;
- (void)AMDidFailToReceiveAd:(AMView *)amView usingBackup:(BOOL)yesOrNo;

/**
 * You can get notified when the transition animation to a new ad is completed
 * so you can make necessary adjustments to the size of the AMView and
 * surrounding views after the animation.
 */
- (void)AMDidAnimateToNewAdIn:(AMView *)amView;
 ...
 * For example, you can replace the ad in AMView after getting this callback
 ...
 * building a game app.
 */
- (void)AMWillPresentFullScreenModal;
- (void)AMDidDismissFullScreenModal;

/**
 * An ad request is a two step process: first the SDK must go to the AM
 ...
 * AMView passed could be null if you had called the AMView class
 * method +startPreFetchingConfigurationDataWithDelegate .
 */
- (void)AMDidReceiveConfig:(AMView *)amView;
 ...
 */
- (UIDeviceOrientation)AMCurrentOrientation;

#pragma mark appearance configurations
- (UIColor *)AMAdBackgroundColor;
- (UIColor *)AMTextColor;
- (UIColor *)AMSecondaryTextColor;
- (UIColor *)backgroundColor DEPRECATED_ATTRIBUTE; // use the one with AM prefix
- (UIColor *)textColor DEPRECATED_ATTRIBUTE; // use the one with AM prefix
...
//- (UIColor *)googleAdSenseAdBackgroundColor; // implement AMAdBackgroundColor or set in server
- (UIColor *)googleAdSenseAdTopBackgroundColor;
- (UIColor *)googleAdSenseAdBorderColor;
- (UIColor *)googleAdSenseAdLinkColor;
//- (UIColor *)googleAdSenseAdTextColor; // implement AMTextColor or set in server
