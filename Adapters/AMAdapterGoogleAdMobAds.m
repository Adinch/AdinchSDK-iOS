...
#import "AMAdapterGoogleAdMobAds.h"
...
@implementation AMAdapterGoogleAdMobAds
...
- (void)getAd {
  ...
  if ([amDelegate respondsToSelector:@selector(AMTestMode)]
      && [amDelegate AMTestMode]) {
    request.testing = YES;
  }

  if ((value = [self helperDelegateValueForSelector:
                @selector(AMAdBackgroundColor)])) {
  ...
  if ((amConfig.locationOn) && (location)) {
  ...
  // Set the frame for this view to match the bounds of the parent AMView.
  GADBannerView *view =
      [[GADBannerView alloc]
       initWithAdSize:GADAdSizeFromCGSize(amView.bounds.size)];
...
- (NSString *)publisherId {
  ...
  if ((delegateSelector) &&
      ([amDelegate respondsToSelector:delegateSelector])) {
    return [amDelegate performSelector:delegateSelector];
  }
